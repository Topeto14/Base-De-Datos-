...
from alumno,alumno_se_matricula_asignatura,asignatura,grado
where alumno.id=alumno_se_matricula_asignatura.id_alumno
and alumno_se_matricula_asignatura.id_asignatura=asignatura.id
and asignatura.id_grado=grado.id
and grado.nombre like'Grado en Ingeniería Informática (Plan 2015)'and sexo like'm';
...
nombre.*/
select profesor.nombre, profesor.apellido1, profesor.apellido2, ifnull(departamento.nombre,'sin departamento')
from profesor
left join departamento
on profesor.id_departamento=departamento.id
order by apellido1, profesor.nombre;
...
from asignatura , alumno_se_matricula_asignatura ,curso_escolar ,alumno
where asignatura.id=alumno_se_matricula_asignatura.id_asignatura
and  alumno.id=alumno_se_matricula_asignatura.id_alumno
and curso_escolar.id=alumno_se_matricula_asignatura.id_curso_escolar
and alumno.nif like'26902806M';
...
select departamento.nombre,profesor.nombre, asignatura.nombre
from profesor,departamento,asignatura,grado
...
select distinct alumno.nombre,alumno.apellido1,alumno.apellido2
from alumno, asignatura,alumno_se_matricula_asignatura,curso_escolar
where  alumno_se_matricula_asignatura.id_alumno=alumno.id
...
left join departamento
on profesor.id_departamento=departamento.id
order by  4 ,apellido1, profesor.nombre;
/*2. Devuelve un listado con los profesores que no están asociados a un
departamento.*/
select distinct profesor.nombre, ifnull(departamento.nombre, 'sin departamento')
from profesor
left join departamento
on profesor.id_departamento=departamento.id
...
select profesor.nombre, count(asignatura.id)
from profesor
...
/* -de aqui saco  las asignaturas que se han impartido*/
select distinct id_asignatura 
from  alumno_se_matricula_asignatura;
/**/
select asignatura.nombre, ifnull(profesor.nombre,'no tiene profesor'), ifnull(departamento.nombre,'sin departamento')
from asignatura
left join profesor
...
select count(id) as 'numero de alumnos'
from alumno
where sexo like 'm';
...
select count(profesor.id) as 'num de profesores', departamento.nombre
...
from asignatura right join grado
...
select grado.nombre, sum(creditos), tipo
from grado, asignatura
where grado.id=asignatura.id_grado
...
select anyo_inicio, count(alumno_se_matricula_asignatura.id_alumno)
...
select profesor.id, profesor.nombre, profesor.apellido1, profesor.apellido2, count(asignatura.id) as 'Nº asignaturas'
...
select *
from alumno
where alumno.fecha_nacimiento=(select max(fecha_nacimiento)
								from alumno);
-- 2. Devuelve un listado con los profesores que no están asociados a un
-- departamento.
...
select profesor.nombre
from profesor
where profesor.id not in (select profesor.id 
					from profesor
                    where profesor.id_departamento is not  null);
                    
    /*las dos son correctaa*/                
select profesor.nombre
from profesor
where profesor.id  in (select profesor.id 
					from profesor
                    where profesor.id_departamento is  null);							
...
/*lo de arriba son los despartamentos que no tinen profesores asociados*/

select departamento.nombre
from departamento
where departamento.id in (select departamento.id
						from departamento
                        left join profesor
                        on profesor.id_departamento=departamento.id
                        group by departamento.id
...
where profesor.id in(select profesor.id
					...
					left join asignatura
					on profesor.id=asignatura.id_profesor
					and profesor.id_departamento is not null
					group by profesor.id
					having count(asignatura.id)=0);
-- 5. Devuelve un listado con las asignaturas que no tienen un profesor asignado.
...
select asignatura.nombre
from asignatura
where asignatura.id in (select asignatura.id
						from profesor right join asignatura
						on profesor.id=asignatura.id_profesor
						group by asignatura.id
						having count(profesor.id)=0);
   
/* esta consultA ME DA TODAS LAS ASIGNATURAS Y EL NUMERO DE PROFESORES*/
select asignatura.nombre, count(profesor.id)
from profesor right join asignatura
on profesor.id=asignatura.id_profesor
group by asignatura.id;
...
/*pendiente de terminar esta consulta*/


SELECT NOMBRE FROM DEPARTAMENTO WHERE DEPARTAMENTO.ID NOT IN(

	SELECT ID_DEPARTAMENTO FROM PROFESOR WHERE ID IN(

		SELECT ID_PROFESOR FROM ASIGNATURA WHERE ID NOT IN(

			SELECT ID_ASIGNATURA FROM ALUMNO_SE_MATRICULA_ASIGNATURA WHERE

				ID_CURSO_ESCOLAR IN (SELECT ID FROM CURSO_ESCOLAR))));